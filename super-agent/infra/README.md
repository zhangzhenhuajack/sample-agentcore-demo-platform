...
部署完成后，可以继续让 Kiro 帮你配置生产级网络和安全：

```
帮我在 EC2 前面加一个 ALB，配置 HTTPS 证书和健康检查
```

```
帮我配置 CloudFront + WAF，限制海外访问并防止 DDoS
```

```
帮我把安全组改为只允许 ALB 访问 EC2，禁止直接公网访问
```

Kiro 会自动创建 ALB/CloudFront 资源、申请 ACM 证书、配置 WAF 规则，并更新安全组确保流量只通过负载均衡器进入。
